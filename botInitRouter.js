...
function getContext(url) {
    ...
    else if (url.match(/twitter\.com\/(.*)\/status\/[0-9]*(.*)$/))
        return Twitter
    else return null
}
...
bot.command('bulkSubmit', async ctx => {
    ...
    ctx.reply(
        `Process finished with ${success} successes and ${
...
async function workURL(url, ctx) {
    try {
        ...
        console.log(
            '\x1b[32m',
            `[${currentTime}]New submit with url ${imgURL} submitted.`,
        )
...
bot.command('submit', async ctx => {
    ...
    const accompanyingObj = JSON.stringify({
        caption: `${
            replacementURL ?? ctx.message.text.split(' ')[1]
        }\n[${postID}]\n${process.env.CHAT_AT}\n\n ${args ?? ''}`,
    })
...
function runQueue() {
    sequelize.models.queue
        ...
        .then(queued => {
            ...
            universalCTXTelegram.telegram.sendPhoto(
                process.env.CHAT_AT,
                imgURL,
                parsedObj,
            )
            queued.destroy().then(() => {
                console.log(
                    '\x1b[32m',
                    '[SUCCESS] Queued item posted and deleted!',
                )
                setTimeout(() => runQueue(), time)
            })
        })
